...
if __name__== '__main__':
    ...
    #    print "Usage : python createPathwayFeatureMatrix.py .. in_file "
    #    exit()
    ...
    with open(sys.argv[3], 'r') as searchfile:
        for l in searchfile:
            ...
            if symbol in uniqueTargets:
                if genePathwayDict.has_key(symbol):
    ...
    for drug1 in sorted(targetdict):
        for drug2 in sorted(targetdict):
            ...
            for t in uniquePathways:
                ...
                if t in pathwayList1:
                    featureStr+="\t1"
                else:
                    featureStr+="\t0"

                if t in pathwayList2:
                    featureStr+="\t1"
                else:
                    featureStr+="\t0"
